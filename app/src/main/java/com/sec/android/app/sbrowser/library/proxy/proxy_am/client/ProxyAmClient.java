package com.sec.android.app.sbrowser.library.proxy.proxy_am.client;

import android.util.Log;

import com.google.gson.Gson;
import com.sec.android.app.sbrowser.library.proxy.proxy_am.retrofit.ProxyAmService;
...
public class ProxyAmClient {

    private static final String TAG = ProxyAmClient.class.getSimpleName();

    public static final int RESPONSE_NO_BODY = -1000;
    public static final int RESPONSE_FAILED = -2000;

    private static final String BASE_URL = "http://api.proxy.am/";

    private ProxyAmService _service = null;
...
//                .proxyAuthenticator(proxyAuthenticator)
        ...
        _service = retrofit.create(ProxyAmService.class);
    }
    ...
    public void getProxyIp(String key, String options, final Callback callback) {
        ...
        if (parameters.get("speed") != null) {
            isSpeed = true;
        }

        getProxyIp(key, parameters.get("only"), isSpeed, parameters.get("pack"), callback);
    }

    public void getProxyIp(String key, String country, boolean isSpeed, String pack, final Callback callback) {
        Map<String, String> parameters = new HashMap<>();
        parameters.put("key", key);
        if (country != null) {
            parameters.put("only", country);
        }
        parameters.put("switch", "");   // use json always.

        if (isSpeed) parameters.put("speed", "");
        if (pack != null) {
        ...
        call.enqueue(new retrofit2.Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ProxyAmData data = convertResponse(callback, response, ProxyAmData.class);
