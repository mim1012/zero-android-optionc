...
import com.sec.android.app.sbrowser.pattern.naver.site.NaverSitePageAction;
import com.sec.android.app.sbrowser.pattern.naver.view.NaverViewAfterPatternAction;

public class NaverInfluencerPatternMessage extends NaverPatternMessage {

    private static final String TAG = NaverInfluencerPatternMessage.class.getSimpleName();
    ...
    private static final int TOUCH_NEXT_BUTTON = RANDOM_SCROLL + 4;
    private static final int SCROLL_BOTTOM = RANDOM_SCROLL + 5;

    private final NaverPatternAction _action;
    private final NaverHomeAction _homeAction;
    private final NaverInfluencerPageAction _viewPageAction;
    ...
    public NaverInfluencerPatternMessage(WebViewManager manager, KeywordItemMoon item) {
        super(manager);
        ...
        _viewPageAction = new NaverInfluencerPageAction(manager.getWebView());
...
//            }
            ...
            case TOUCH_CONTENT: {
                Log.d(TAG, "# 네이버 인플루언서 컨텐츠 검사");
                InsideData insideData = _viewPageAction.getParentContentUrlInsideData(_url);
                if (insideData != null) {
                    if (insideData.isInside()) {
                        InsideData insideData2 = _viewPageAction.getContentUrlInsideData(_url);
                        if (insideData2 != null) {
                            if (insideData2.isInside()) {
                                Log.d(TAG, "# 네이버 인플루언서 컨텐츠 터치");
                                if (_viewPageAction.touchContentUrl(_url)) {
                                    webViewLoading(msg);
                                } else {
                                    Log.d(TAG, "# 네이버 인플루언서 컨텐츠 터치 실패로 패턴종료2.");
                                    _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
                                }
                            } else if (insideData2.inside > 0) {
                                Log.d(TAG, "# 화면에 안보여서 아래로 스크롤2");
                                _swipeAction.swipeDown();
                                _handler.sendEmptyMessageDelayed(msg.what, MathHelper.randomRange(1000, 2000));
                            } else {
                                Log.d(TAG, "# 화면에 안보여서 위로 스크롤2");
                                _swipeAction.swipeUp();
                                _handler.sendEmptyMessageDelayed(msg.what, MathHelper.randomRange(1000, 2000));
                            }
                        } else {
                            Log.d(TAG, "# 네이버 인플루언서 컨텐츠 터치 실패로 패턴종료.");
                            _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
                        }
                    ...
                    if (_currentPage <= 1) {
                        ++_currentPage;
                        Log.d(TAG, "# 네이버 인플루언서 컨텐츠 못찾아서 다음으로...");
                        sendMessageDelayed(TOUCH_MORE_BUTTON, 100);
                    } else {
                        if (_viewPageAction.getNodeCount() < 100) {
                            if (_scrollCount < 10) {
                                Log.d(TAG, "# 네이버 인플루언서 컨텐츠 못찾아서 다음으로 스크롤.. " + _scrollCount);
                                ++_scrollCount;
                                _handler.sendEmptyMessageDelayed(SCROLL_BOTTOM, MathHelper.randomRange(1000, 2000));
                            } else {
                                Log.d(TAG, "# 네이버 인플루언서 컨텐츠 " + _viewPageAction.getNodeCount() + "위 내에 없어서 패턴종료.");
                                _handler.sendEmptyMessageDelayed(END_PATTERN, 5000);
                            }
                        } else {
                            Log.d(TAG, "# 네이버 인플루언서 컨텐츠 " + _viewPageAction.getNodeCount() + "위 내에 없어서 패턴종료.");
...
//                    }
//                } else {
//                    Log.d(TAG, "# 플레이스 순위 검사 실패로 5초후 다시 시도..." + _retryCount);
//                    if (!resendMessageDelayed(msg.what, 5000, 3)) {
//                        Log.d(TAG, "# 플레이스 순위 검사에 실패로 패턴종료.");
...
//            }
            ...
            case TOUCH_MORE_BUTTON: {
                Log.d(TAG, "# 더보기 버튼 검사");
                InsideData insideData = _viewPageAction.getMoreButtonInsideData();
                if (insideData != null) {
                    if (insideData.isInside()) {
                        Log.d(TAG, "# 더보기 버튼 터치");
                        if (_viewPageAction.touchButton(NaverSitePageAction.BUTTON_MORE)) {
                            webViewLoading(msg);
                        } else {
...
//                        _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
//                    } else if (Integer.parseInt(page) > 5) {
//                        Log.d(TAG, "# 5페이지 초과로 패턴종료.");
//                        _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
//                    } else {
//                        if (insideData.isInside()) {
//                            Log.d(TAG, "# 다음 버튼 터치");
//                            if (_viewPageAction.touchButton(NaverSitePageAction.BUTTON_NEXT)) {
//                                webViewLoading(msg);
//                            } else {
...
//            }
            ...
            case TOUCH_CONTENT: {
                Log.d(TAG, "# 네이버 인플루언서 컨텐츠 터치 후 동작");
//                _result = 1;
