...
import com.sec.android.app.sbrowser.pattern.ohouse.OHousePatternMessage;
import com.sec.android.app.sbrowser.pattern.ohouse.action.OHouseHomeAction;
import com.sec.android.app.sbrowser.pattern.ohouse.action.OHousePatternAction;

import java.io.UnsupportedEncodingException;
import java.net.URLEncoder;

public class OHouseShopPatternMessage extends OHousePatternMessage {

    private static final String TAG = OHouseShopPatternMessage.class.getSimpleName();

    private static final int MAX_SCROLL_COUNT = 40;

    protected static final int GO_HOME_DIRECT = 40;
    protected static final int GO_SHOP_HOME = 41;
    protected static final int GO_SHOP_HOME_DIRECT = 42;
    ...
    private final OHousePatternAction _action;
    private final OHouseHomeAction _homeAction;
    private final OHouseShopPageAction _shopPageAction;
//    private final NaverViewAfterPatternAction _afterPatternAction;
    ...
    public OHouseShopPatternMessage(WebViewManager manager, KeywordItemMoon item) {
        super(manager);
        _item = item;
        _keyword = item.keyword;
        _code = item.mid1;

        _action = new OHousePatternAction(manager.getWebView());
        _homeAction = new OHouseHomeAction(manager.getWebView());
        _shopPageAction = new OHouseShopPageAction(manager.getWebView());
        ...
        switch (msg.what) {
            case START_PATTERN: {
              ...
              //  _handler.sendEmptyMessage(FIND_KEYWORD);
                ...
                } else if (_item.item.shopHome == 3 || _item.item.shopHome == 4) {
            ...
            case GO_HOME: {
                Log.d(TAG, "# 오늘의집 홈으로 이동");
                webViewLoad(msg, HOME_URL);
                break;
            }

            case GO_SHOP_HOME: {
                Log.d(TAG, "# 오늘의집 쇼핑 홈으로 이동");
                webViewLoad(msg, "https://ohou.se/store");
                break;
            }

            case GO_HOME_DIRECT: {
                Log.d(TAG, "# 오늘의집 홈 검색어로 이동");
                try {
                ...
                webViewLoad(msg, "https://ohou.se/search/index?query=" + _keyword + "&search_affect_type=Typing");
                break;
            }
            case GO_SHOP_HOME_DIRECT: {
                Log.d(TAG, "# 오늘의집 쇼핑홈 검색어로 이동");
                _step = 1;
                try {
                    _keyword = URLEncoder.encode(_keyword, "UTF-8");
                } catch (UnsupportedEncodingException e) {
                ...
                webViewLoad(msg, "https://ohou.se/productions/feed?type=store&query=" + _keyword + "");
                break;
            }
            ...
            case TOUCH_SEARCH_BAR: {
                Log.d(TAG, "# 검색버튼 터치");
                if (_homeAction.touchButton(OHouseHomeAction.BUTTON_SEARCH)) {
...
//                            webViewLoading(msg);
                            ...
                            Log.d(TAG, "# 오늘의집 컨텐츠 터치 실패로 패턴종료.");
                            _workCode = 230021;
                            _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
                        }
                    ...
                    if (_step < 1) {    // 모통홈 로직
                        ...
                        if (_scrollCount < MAX_SCROLL_COUNT) {
                            Log.d(TAG, "# 오늘의집 컨텐츠 못찾아서 아래로 스크롤..." + _scrollCount);
                            ++_scrollCount;
                            _swipeAction.swipeDownFast(110, 200);
                            _handler.sendEmptyMessageDelayed(msg.what, MathHelper.randomRange(1000, 2000));
                        } else {
                            Log.d(TAG, "# 오늘의집 컨텐츠 못찾아서 패턴 종료..." + _scrollCount);
                            _workCode = 230023;
                            _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
                        }
                    }
            ...
            case TOUCH_MORE_BUTTON: {
                ...
                if (insideData != null) {
                    if (insideData.isInside()) {
                        Log.d(TAG, "# 더보기 버튼 터치");
                        if (_shopPageAction.touchButton(OHouseShopPageAction.BUTTON_MORE)) {
//                            webViewLoading(msg);
...
//                _afterPatternAction.endPattern();
    ...
    public void onPageLoaded(String url) {
        ...
        switch (_lastMessage) {
            ...
            case GO_SHOP_HOME_DIRECT: {
                Log.d(TAG, "# 오늘의집 쇼핑 홈 검색어로 이동 후 동작");
                _findBarCount = 0;
                _nextPopupMessage = TOUCH_CONTENT;
                _handler.sendEmptyMessageDelayed(TOUCH_MOBILE_WEB_POPUP, MathHelper.randomRange(5000, 6000));
                break;
