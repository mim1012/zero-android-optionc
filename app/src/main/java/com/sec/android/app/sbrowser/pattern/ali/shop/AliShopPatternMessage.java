...
import com.sec.android.app.sbrowser.pattern.ably.shop.AblyShopPageAction;
...
public class AliShopPatternMessage extends AliPatternMessage {

    private static final String TAG = AliShopPatternMessage.class.getSimpleName();

    private static final int MAX_FIND_COUNT = 60;
    ...
    private final AliShopPageAction _shopPageAction;
//    private final NaverViewAfterPatternAction _afterPatternAction;
    ...
    public AliShopPatternMessage(WebViewManager manager, KeywordItemMoon item) {
        super(manager);
        _item = item;
        ...
        _shopPageAction = new AliShopPageAction(manager.getWebView());
//        _afterPatternAction = new NaverViewAfterPatternAction(manager.getWebView(), _url);
    ...
    public void onHandleMessage(Handler handler, Message msg) {
        ...
        switch (msg.what) {
            ...
            case GO_SEARCH_HOME: {
                Log.d(TAG, "# 알리 검색 홈으로 이동");
//                webViewLoad(msg, "https://m.a-bly.com/search");
                break;
            }

            case GO_HOME_DIRECT: {
                Log.d(TAG, "# 알리 홈 검색어로 이동");
                try {
...
//            }
            ...
            case TOUCH_CONTENT: {
                if (_findCount >= MAX_FIND_COUNT) {
                    Log.d(TAG, "# 알리 컨텐츠 터치 실패로 패턴종료.");
                    _workCode = 430022;
                    _handler.sendEmptyMessageDelayed(END_PATTERN, 1000);
                    break;
                }

                Log.d(TAG, "# 알리 컨텐츠 검사: " + _findCount);
                ++_findCount;
...
//                            _handler.sendEmptyMessageDelayed(RANDOM_SCROLL, MathHelper.randomRange(5000, 7000));
                        } else {
                            Log.d(TAG, "# 알리 컨텐츠 터치 실패로 패턴종료.");
                            _workCode = 430021;
                            _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
                    ...
                    if (_step < 1) {    // 모통홈 로직
                        ...
                        if (_scrollCount < MAX_SCROLL_COUNT) {
                            Log.d(TAG, "# 알리 컨텐츠 못찾아서 아래로 스크롤..." + _scrollCount);
                            ++_scrollCount;
                            _swipeAction.swipeDownFast(110, 200);
                            _handler.sendEmptyMessageDelayed(msg.what, MathHelper.randomRange(1000, 2000));
                        } else {
                            Log.d(TAG, "# 알리 컨텐츠 못찾아서 패턴 종료..." + _scrollCount);
                            _workCode = 430023;
                            _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
                        }
                    }
