...
import com.sec.android.app.sbrowser.pattern.naver.shop.NaverShopSearchBarClearPatternAction;

import java.io.UnsupportedEncodingException;
import java.net.URLEncoder;
import java.util.ArrayList;

public class NaverActivePatternMessage extends NaverPatternMessage {

    private static final String TAG = NaverActivePatternMessage.class.getSimpleName();

    private static final int MAX_PAGE_COUNT = 0;
    ...
    protected static final int GO_SHOP_CONTENT_URL = 43;
    private static final int RANDOM_SCROLL = 50;
    private static final int TOUCH_BACK_BUTTON = RANDOM_SCROLL + 1;
    private static final int TOUCH_MAIL_BUTTON = TOUCH_BACK_BUTTON + 1;
    private static final int TOUCH_CONTENT = TOUCH_MAIL_BUTTON + 2;
    private static final int TOUCH_GO_TO_BUY_BUTTON = TOUCH_CONTENT + 3;
    private static final int TOUCH_TOP_DOTS_BUTTON = TOUCH_CONTENT + 4;
    private static final int TOUCH_TOP_SHOPPING_BUTTON = TOUCH_CONTENT + 5;
    private static final int TOUCH_MORE_BUTTON = TOUCH_CONTENT + 6;
    private static final int TOUCH_MORE2_BUTTON = TOUCH_CONTENT + 7;
    private static final int TOUCH_HOME_NEXT_BUTTON = TOUCH_CONTENT + 8;
    private static final int TOUCH_HOME_PREV_BUTTON = TOUCH_CONTENT + 9;
    ...
    private static final int TOUCH_HOME_MAIL_BUTTON = TOUCH_AFTER_POPULAR + 1;
    private static final int TOUCH_HOME_PAN_BUTTON = TOUCH_HOME_MAIL_BUTTON + 1;
    ...
    private final NaverMailActivePatternMessage _mailMessage;
...
//    protected final NaverShopPatternMessage _subNaverShopPatternMessage;
    ...
    public NaverActivePatternMessage(WebViewManager manager, KeywordItemMoon item) {
        super(manager);
        _item = item;
...
//            _keyword = storeName + productName;
        ...
        _mailMessage = new NaverMailActivePatternMessage(manager, item);
...
//            _subNaverShopPatternMessage = new NaverShopPatternMessageAll(manager, item);
    ...
    public void onHandleMessage(Handler handler, Message msg) {
        ...
        _mailMessage.onHandleMessage(handler, msg);

//        _subNaverShopPatternMessage.onHandleMessage(handler, msg);

        switch (msg.what) {
            case START_PATTERN: {
                Log.d(TAG, "# 네이버 활동 패턴 시작");
                _searchStep = 0;
                _randomClickWorkCount = 0;
                _nnbData = UserManager.getInstance().nnbData;

                _webViewManager.getWebView().post(() -> {
                    String currentUrl = _webViewManager.getWebView().getUrl();

                    if (currentUrl.startsWith(Config.NAVER_HOME_MOBILE_URL) || currentUrl.startsWith("https://www.naver.com")) {
                        _handler.sendEmptyMessageDelayed(TOUCH_HOME_MAIL_BUTTON, MathHelper.randomRange(5500, 7500));
                    } else {
            ...
            case TOUCH_NEXT_BUTTON: {
                ...
                if (insideData != null) {
                    ...
                    if (page == null) {
                        ...
                        _workCode = 110041;
                        _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
                    } else if (Integer.parseInt(page) > MAX_PAGE_COUNT) {
...
//                _searchBarClearPatternAction.keyword = _step == 0 ? _item.search : _item.target;
            ...
            case TOUCH_HOME_MAIL_BUTTON: {
                Log.d(TAG, "# 홈 메일 버튼 터치");
                runHomeButtonTouch(msg, NaverHomeAction.BUTTON_MAIL, "메일");
                break;
            }

            case NaverMailActivePatternMessage.END_MAIL_PATTERN: {
...
//                }
            ...
            case TOUCH_HOME_MAIL_BUTTON: {
                Log.d(TAG, "# 홈 메일 버튼 터치 후 동작");
                // 액션: 메일 클릭, 스크롤, 편지함 탭 이동, 메뉴버튼 액션
                _handler.sendEmptyMessageDelayed(NaverMailActivePatternMessage.START_MAIL_PATTERN, MathHelper.randomRange(3000, 5000));
//                runHomeButtonTouch(msg, NaverHomeAction.BUTTON_MAIL, "메일");
                break;
            }
...
//                _handler.sendEmptyMessageDelayed(PICK_HOME_PAN_CONTENT, MathHelper.randomRange(3000, 5000));
    ...
    public void runHomeButtonTouch(Message msg, int type, String name) {
        _homePickCount = 0;

        if (_homeAction.touchButton(type)) {
            webViewLoading(msg);
        } else {
            if (_homeWaitCount < 10) {
                ++_homeWaitCount;
                ...
                if (_homeWaitCount % 4 == 0) {
                    Log.d(TAG, "# 홈 " + name + " 버튼 터치 실패로 새로고침... " + _homeWaitCount);
                    _handler.sendEmptyMessageDelayed(RUN_NEXT, MathHelper.randomRange(3500, 5000));
                } else {
                    Log.d(TAG, "# 홈 " + name + " 버튼 터치 실패로 3초 후 다시 시도..." + _homeWaitCount);
                    _handler.sendEmptyMessageDelayed(msg.what, 3000);
                }
            } else {
                // 여기에 재시도 만들것. 실패하면 다시 홈로드.
                Log.d(TAG, "# 홈 " + name + " 버튼 터치에 실패해서 패턴종료.");
                _workCode = 140001 + type;
                _handler.sendEmptyMessageDelayed(END_PATTERN, 3000);
            }
        }
    }
